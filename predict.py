...
def greedy_search(loaded_array):
    ...
    """
    inf_encoder_model, inf_decoder_model, tokenizer = (
        settings.inf_encoder_model,
        settings.inf_decoder_model,
        settings.tokenizer,
    )
    inv_map = index_to_word(tokenizer)
    states_value = inf_encoder_model.predict(
        loaded_array.reshape(-1, config.time_steps_encoder, config.num_encoder_tokens)
    )
...
def test(images):
    ...
    sentence_predicted = greedy_search(
        X_test.reshape((-1, config.time_steps_encoder, config.num_encoder_tokens))
    )
    # re-init max prob
